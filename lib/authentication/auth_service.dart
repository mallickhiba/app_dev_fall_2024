...
class AuthService {
  ...
  Future<bool> _accountExists(
      String email, String signInType, bool isSignIn) async {
    QuerySnapshot snapshot;
    if (isSignIn) {
      snapshot = await _store
          .collection('users')
          .where("email", isEqualTo: email)
          .where("sign_in_type", isEqualTo: signInType)
          .get();
  ...
  Future<User?> signInWithEmailAndPassword(
    ...
    if (!accountAlrExists) {
      //if account does not exist its not going to let you sign in
      return null;
    }
  ...
  Future<User?> signUpWithEmailAndPassword(
    ...
    if (accountAlrExists) {
      //if account already exists its not going to let you sign up
      return null;
    }
  ...
  Future<String> signInWithGoogle() async {
    try {
      final googleAccount = await GoogleSignIn()
          .signIn(); //initiates the sign in process. returns a GoogleSignInAccount object if successful, and null if not.

      final googleAuth = await googleAccount
          ?.authentication; //contains tokens needed to authenticate the user with firebase.

      if (googleAccount == null) {
        return 'user did not select account';
      }

      bool accountAlrExists =
          await _accountExists(googleAccount.email, "google", true);
      ...
      final credential = GoogleAuthProvider.credential(
          //created a credential here that will be used to login to firebase.
  ...
  Future<String> signUpWithGoogle() async {
    try {
      ...
      bool accountAlrExists =
          await _accountExists(googleAccount.email, "google", false);

      if (accountAlrExists) {
        // if account does exist its not going to let you sign up
        GoogleSignIn().signOut();
